...
const getDisplayLabel = (status: string | null) => {
  ...
  return estadoEntregaOrder.find(
...
export default function PedidoEstadoActions({
  ...
  const nextStates = useMemo(() => {
    ...
    return [estadoEntregaOrder[currentIndex + 1]];
  }, [currentIndex]);
    ...
    <div className="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm shadow-slate-200/50">
      <div className="mb-4">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Cambia el estado de la entrega y avisa sobre el progreso. No se puede regresar a estados anteriores.
        </p>
      </div>
      ...
      <div className="grid gap-3">
        {nextStates.length > 0 ? (
          ...
          <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
            El estado de entrega ya está en su etapa final y no se puede actualizar más.
          </div>
        )}
      </div>

...
export default function CheckoutPage({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const successUrl = new URL(
      "/user/usuario/compras/exito",
      window.location.origin
    );
    successUrl.searchParams.set("entrega", tipoEntrega ?? "");
    successUrl.searchParams.set("metodo", "Tarjeta");
    successUrl.searchParams.set("total", String(amount));
    if (fechaHoraRetiro) {
      successUrl.searchParams.set("retiro", fechaHoraRetiro);
    }
  ...
  return (
    ...
    >
      ...
      <div className="sticky bottom-0 border-t border-indigo-100 bg-white p-4">
        <button
          type="submit"
